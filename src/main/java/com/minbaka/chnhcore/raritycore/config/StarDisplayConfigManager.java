                    ...
                    if (customObj.has("strings")) {
                        ...
                        for (String key : stringsObj.keySet()) {
                            try {
                                ...
                                org.slf4j.LoggerFactory.getLogger("RarityCore").warn("Invalid rarity key in custom star strings: {}", key);
                            }
                        }
                    }
                    ...
                    if (customObj.has("specialRarityTexts")) {
                        ...
                        for (String key : specialTextsObj.keySet()) {
                            try {
                                ...
                                org.slf4j.LoggerFactory.getLogger("RarityCore").warn("Invalid rarity key in special rarity texts: {}", key);
                            }
                        }
                    }
                }
                
                org.slf4j.LoggerFactory.getLogger("RarityCore").info("Loaded star display config: enabled={}, mode={}, repeatChar='{}', customStrings={}, specialRarityTexts={}", 
                    enableStarDisplay, starMode, repeatCharacter, customStarStrings.size(), customSpecialRarityTexts.size());
            } else {
                // 如果没有starDisplay配置,使用默认值
                resetToDefaults();
                org.slf4j.LoggerFactory.getLogger("RarityCore").info("No star display config found, using defaults");
            }
        } catch (Exception e) {
            org.slf4j.LoggerFactory.getLogger("RarityCore").error("Error loading star display config, using defaults: {}", e.getMessage());
            // 出错时使用默认值
            resetToDefaults();
        }
    }
