package com.minbaka.chnhcore.raritycore.cache;
...
import com.minbaka.chnhcore.raritycore.cache.CacheMetrics.CacheType;
...
public class DualCacheManager {
    ...
    public static void initialize() {
        ...
        createCaches();
        preloadIdCache();
        org.slf4j.LoggerFactory.getLogger("RarityCore").info("Dual cache system initialized - ID cache: {}, NBT cache: {}", 
    ...
    private static Cache<String, Integer> createNbtCache() {
        ...
        org.slf4j.LoggerFactory.getLogger("RarityCore").info("NBT cache created with dynamic capacity: {} entries", actualNbtCacheSize);
        return cache;
    }
    ...
    private static void preloadIdCache() {
        ...
        try {
            // 使用并行流加速预加载过程
            com.minbaka.chnhcore.raritycore.registry.RarityRegistry.ITEM_RARITY_MAP.entrySet().parallelStream().forEach(entry -> {
                try {
                    idCache.put(entry.getKey(), entry.getValue());
                    successCount.incrementAndGet();
                } catch (Throwable e) {
                    errorCount.incrementAndGet();
                    org.slf4j.LoggerFactory.getLogger("RarityCore").debug("Failed to cache rarity for item: {}", entry.getKey(), e);
                }
            });
        } catch (Throwable e) {
            org.slf4j.LoggerFactory.getLogger("RarityCore").error("Error loading ITEM_RARITY_MAP during cache preload", e);
        }
            
        org.slf4j.LoggerFactory.getLogger("RarityCore").info("ID cache preloaded: {} items from config, {} items failed", 
    ...
    public static void handleConfigReload() {
        ...
        try {
            ...
            if (idCache == null || nbtCache == null) {
                org.slf4j.LoggerFactory.getLogger("RarityCore").warn("DualCacheManager not initialized yet, skipping reload");
                return;
            }
            ...
            org.slf4j.LoggerFactory.getLogger("RarityCore").info("Dual cache system reloaded - ID cache: {}, NBT cache: {}",
                idCache.size(), nbtCache.size());
        } finally {
            isReloading = false;
        }
    ...
    public static Integer getCachedRarityByItem(Item item) {
        ...
        ResourceLocation idKey = net.minecraft.core.registries.BuiltInRegistries.ITEM.getKey(item);
        if (idKey == null || idKey.equals(net.minecraft.core.registries.BuiltInRegistries.ITEM.getDefaultKey())) return null;
