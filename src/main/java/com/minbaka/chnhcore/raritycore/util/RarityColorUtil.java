...
public class RarityColorUtil {
    ...
    public static int parseRgbColor(String hex) {
        if (hex == null || hex.isEmpty()) {
            org.slf4j.LoggerFactory.getLogger("RarityCore").warn("parseRgbColor received null/empty input, falling back to default gray");
            return DEFAULT_RGB_COLOR;
        }
        try {
            String cleaned = hex.startsWith("#") ? hex.substring(1) : hex;
            if (cleaned.length() != 6) {
                org.slf4j.LoggerFactory.getLogger("RarityCore").warn("parseRgbColor received invalid hex length: '{}', falling back to default gray", hex);
                return DEFAULT_RGB_COLOR;
            }
            return Integer.parseInt(cleaned, 16);
        } catch (NumberFormatException e) {
            org.slf4j.LoggerFactory.getLogger("RarityCore").warn("parseRgbColor failed to parse '{}', falling back to default gray: {}", hex, e.getMessage());
            return DEFAULT_RGB_COLOR;
        }
    }
