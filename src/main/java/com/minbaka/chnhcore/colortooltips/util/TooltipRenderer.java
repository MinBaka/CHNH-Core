package com.minbaka.chnhcore.colortooltips.util;

import com.mojang.blaze3d.systems.RenderSystem;
import net.minecraft.client.gui.GuiGraphics;
import net.minecraft.client.renderer.RenderType;
import net.minecraft.Util;
import com.minbaka.chnhcore.colortooltips.Config;
...
public class TooltipRenderer {
    ...
    public static void drawGradientScrollingBorder(GuiGraphics graphics, int x, int y, int width, int height, int rarityColor, int variationSeedColor, float fadeAlpha, int targetWidth, int targetHeight) {
        ...
        for (int i = 0; i < segmentsPerEdge; i++) {
            ...
            consumer.addVertex(matrix, x1, y, 0).setColor(c1[0], c1[1], c1[2], c1[3]);
            consumer.addVertex(matrix, x1, y + 1, 0).setColor(c1[0], c1[1], c1[2], c1[3]);
            consumer.addVertex(matrix, x2, y + 1, 0).setColor(c2[0], c2[1], c2[2], c2[3]);
            consumer.addVertex(matrix, x2, y, 0).setColor(c2[0], c2[1], c2[2], c2[3]);
        }
        ...
        for (int i = 0; i < segmentsPerEdge; i++) {
            ...
            unpackColor(computeGradientColor(d1 * targetPhaseScale, colors), fadeAlpha, c1);
            unpackColor(computeGradientColor(d2 * targetPhaseScale, colors), fadeAlpha, c2);
            consumer.addVertex(matrix, x + width - 1, y1, 0).setColor(c1[0], c1[1], c1[2], c1[3]);
            consumer.addVertex(matrix, x + width - 1, y2, 0).setColor(c2[0], c2[1], c2[2], c2[3]);
            consumer.addVertex(matrix, x + width, y2, 0).setColor(c2[0], c2[1], c2[2], c2[3]);
            consumer.addVertex(matrix, x + width, y1, 0).setColor(c1[0], c1[1], c1[2], c1[3]);
        }
        ...
        for (int i = 0; i < segmentsPerEdge; i++) {
            ...
            unpackColor(computeGradientColor(d1 * targetPhaseScale, colors), fadeAlpha, c1);
            unpackColor(computeGradientColor(d2 * targetPhaseScale, colors), fadeAlpha, c2);
            consumer.addVertex(matrix, x2, y + height - 1, 0).setColor(c2[0], c2[1], c2[2], c2[3]);
            consumer.addVertex(matrix, x2, y + height, 0).setColor(c2[0], c2[1], c2[2], c2[3]);
            consumer.addVertex(matrix, x1, y + height, 0).setColor(c1[0], c1[1], c1[2], c1[3]);
            consumer.addVertex(matrix, x1, y + height - 1, 0).setColor(c1[0], c1[1], c1[2], c1[3]);
        }
        ...
        for (int i = 0; i < segmentsPerEdge; i++) {
            ...
            unpackColor(computeGradientColor(d1 * targetPhaseScale, colors), fadeAlpha, c1);
            unpackColor(computeGradientColor(d2 * targetPhaseScale, colors), fadeAlpha, c2);
            consumer.addVertex(matrix, x, y2, 0).setColor(c2[0], c2[1], c2[2], c2[3]);
            consumer.addVertex(matrix, x, y1, 0).setColor(c1[0], c1[1], c1[2], c1[3]);
            consumer.addVertex(matrix, x + 1, y1, 0).setColor(c1[0], c1[1], c1[2], c1[3]);
            consumer.addVertex(matrix, x + 1, y2, 0).setColor(c2[0], c2[1], c2[2], c2[3]);
        }
    ...
    public static void drawGradientTitleBar(GuiGraphics graphics, int x, int y, int width, int height, int rarityColor, int variationSeedColor, float fadeAlpha, int targetWidth, int targetHeight, int titleBarHeight) {
        ...
        for (int i = 0; i < segments; i++) {
            ...
            consumer.addVertex(matrix, x1, barY1, 0).setColor(r1, g1, b1, a1);
            consumer.addVertex(matrix, x1, barY2, 0).setColor(r1, g1, b1, a1);
            consumer.addVertex(matrix, x2, barY2, 0).setColor(r2, g2, b2, a2);
            consumer.addVertex(matrix, x2, barY1, 0).setColor(r2, g2, b2, a2);
        }
        ...
        java.util.function.BiConsumer<java.util.function.Function<Float, float[]>, Float> drawTrail = (posFunc, currentProgress) -> {
            for (int i = 0; i < tailSegments; i++) {
                ...
                if (len > 0.001f) {
                    ...
                    consumer.addVertex(matrix, pos2[0] + nx * thickness, pos2[1] + ny * thickness, 0).setColor(1f, 1f, 1f, alpha2);
                    consumer.addVertex(matrix, pos2[0] - nx * thickness, pos2[1] - ny * thickness, 0).setColor(1f, 1f, 1f, alpha2);
                    consumer.addVertex(matrix, pos1[0] - nx * thickness, pos1[1] - ny * thickness, 0).setColor(1f, 1f, 1f, alpha1);
                    consumer.addVertex(matrix, pos1[0] + nx * thickness, pos1[1] + ny * thickness, 0).setColor(1f, 1f, 1f, alpha1);
                } else if (i == 0) {
                    // 对于非常小的移动或者头部点,直接绘制一个小方块
                    consumer.addVertex(matrix, pos1[0] + 1, pos1[1] - 1, 0).setColor(1f, 1f, 1f, alpha1);
                    consumer.addVertex(matrix, pos1[0] + 1, pos1[1] + 1, 0).setColor(1f, 1f, 1f, alpha1);
                    consumer.addVertex(matrix, pos1[0] - 1, pos1[1] + 1, 0).setColor(1f, 1f, 1f, alpha1);
                    consumer.addVertex(matrix, pos1[0] - 1, pos1[1] - 1, 0).setColor(1f, 1f, 1f, alpha1);
                }
